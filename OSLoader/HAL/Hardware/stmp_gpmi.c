...
{
    uint32_t DST, DH, AST, DT;


    DeviceTimeOutCycles = nsToCycles(80000000, 1000000000UL / (GPMIFreq / 4096UL), 0);  //80ms
    //DeviceTimeOutCycles = 0xFFFF;

    DT = DeviceTimeOutCycles;
    DST = nsToCycles(timing.DataSetup_ns, 1000000000UL / GPMIFreq, 1);
    DH = nsToCycles(timing.DataHold_ns, 1000000000UL / GPMIFreq, 1);
    AST = nsToCycles(timing.DataSetup_ns, 1000000000UL / GPMIFreq, 0);
    
    INFO("GPMI CYC, DST:%ld, DH:%ld, AST:%ld, DT:%ld\n", DST, DH, AST, DT);
